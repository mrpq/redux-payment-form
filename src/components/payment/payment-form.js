...
const getDetectedCard = (detectedCreditCard, acceptedCards) => {
  if (
    detectedCreditCard.length === 1 &&
    isCardSupported(acceptedCards, detectedCreditCard[0].type)
  ) {
...
const validateCreditCard = (value, acceptedCards) => {
  ...
  if (
    has(detectedCreditCard, "lengths") &&
    detectedCreditCard.lengths.indexOf(value.length) === -1
  ) {
...
const validate = (values, props) => {
  ...
  const securityCodeLength = get(
    getDetectedCard(detectedCreditCard, props.acceptedCards),
    "code.size"
  );
...
const renderCreditCardPictures = (supportedCards, cardTypeName) =>
  supportedCards.map(
    (creditCard, index) =>
      cardTypeName === creditCard.name && (
        <img key={index} width="40" src={creditCard.image} alt={cardTypeName} />
      )
  );
...
const renderCreditCardField = ({
  ...
  return (
    <div className={`form-group ${touched && error && `has-error`}`}>
      <label htmlFor={label} className="col-lg-3 control-label">
        {label}
      </label>
      ...
      <div className="col-lg-3">
        <select className="form-control" {...input}>
          ...
          {options.map((option, index) => (
            <option key={index} value={valueInterceptor(index)}>
              {option}
            </option>
          ))}
        </select>
        ...
        <fieldset>
          ...
          <FormSection name="payment">
            ...
            <Field
              ...
              label="Expiry Month"
              options={monthNames}
              valueInterceptor={(index) => index + 1}
            ...
            <Field
              ...
              label="Expiry Year"
              options={expiryYears}
              valueInterceptor={(index) => index}
              component={renderDateFieldFull}
            />
            <Field
              name="securityCode"
              type="text"
              label={securityCodeName ? securityCodeName : "Security Code"}
              fieldGridSize="4"
              component={renderTextField}
            />
          </FormSection>
